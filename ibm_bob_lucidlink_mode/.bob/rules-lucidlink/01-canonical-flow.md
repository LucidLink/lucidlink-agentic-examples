...
- The MCP server auto-links a filespace on the first file operation: the
  configured default (`LUCIDLINK_FILESPACE`) or the only filespace the
  account can see. You usually do NOT need to link explicitly.
- If an operation reports that no/ambiguous filespace is linked: call
  `list_filespaces`, then `link_filespace(name=...)`, then confirm with
  `current_filespace`.
